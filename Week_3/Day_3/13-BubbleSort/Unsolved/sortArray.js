...
for (key in numbers) {
  if (key !== 0) {
    var previous = numbers[key -1];
    var current = numbers[key];
    if (current < previous) {
      numbers[key] = previous;
      numbers[key - 1] = current;
        moves = true;
...
function bubbleSort(arr) {
  ...
  // everytime we iterate over the array, we know at least the last value has been sorted, so we don’t have to iterate to that index again
...
function runBubbleSort(element) {
  do {
    bubbleSort(test);
  } while (!sorted);

  addArrayToDom(test, ‘result’);
}

function addArrayToDom(myArray, element) {
  var numbersToAdd = ‘’;
  var delimiter = ‘, ’;
  for (var i = 0; i < myArray.length; i++) {

    // check for the last element on the array
    if (i === myArray.length - 1) {
      numbersToAdd += myArray[i]
    } else {
      numbersToAdd += myArray[i] + delimiter;
    }
  }

  document.getElementById(element).innerHTML = numbersToAdd;
}

addArrayToDom(test, ‘start’);
